...
with st.sidebar:
    ...
    min_price = st.number_input(
        "Min Price ($)",
        value=st.session_state.get("default_min_price", MIN_PRICE),
        min_value=0.5, step=0.5,
    )
    min_vol = st.number_input(
        "Min Avg Volume",
        value=int(st.session_state.get("default_min_volume", MIN_VOLUME)),
        min_value=10_000, step=50_000,
    )
    min_score = st.number_input(
        "Min Overall Score",
        value=int(st.session_state.get("default_min_score", MIN_SCORE)),
        min_value=0, max_value=100, step=5,
    )
    min_ema = st.number_input(
        "Min EMA Score",
        value=int(st.session_state.get("default_min_ema", MIN_EMA_SCORE)),
        min_value=0, max_value=100, step=5,
    )
    ...
    theme_choice = st.selectbox(
        "Investment Theme",
        ["All Stocks"] + list(INVESTMENT_THEMES.keys()),
    )
...
if st.session_state.get("scanner_ran"):
    ...
    if theme_choice != "All Stocks":
        filters["theme_symbols"] = INVESTMENT_THEMES[theme_choice]
...
if df is not None and not df.empty:
    ...
    # Format for display
    display_df = df[["ticker", "name", "price", "score", "ema_score",
                      "breakout_score", "institutional_score", "rsi",
                      "momentum_5d", "momentum_20d", "volume_ratio",
                      "flow_signal", "breakout_pattern"]].copy()
